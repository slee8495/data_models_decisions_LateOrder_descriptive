...
source("pre_data_processing.R")

specific_date <- as.Date("2024-02-22")
...
server <- function(input, output, session) {
  ...
  max_order_date <- reactive({
    max(cleaned_default_data()$OrderDate, na.rm = TRUE)
  })
